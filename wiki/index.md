...
- [0002-flat-cmd-package-layout](decisions/0002-flat-cmd-package-layout.md) — flat `cmd/pecunia/` package, no deep tree
...
- [0004-dev-build-isolated-by-ldflags](decisions/0004-dev-build-isolated-by-ldflags.md) — dev build with seeded `pecunia.dev.db`, isolated via ldflags
...
- [0012-summary-composes-existing-stores](decisions/0012-summary-composes-existing-stores.md) — `pecunia s` composes existing stores, no new tables
...
- [0017-mcp-server-exposes-every-module-as-a-tool](decisions/0017-mcp-server-exposes-every-module-as-a-tool.md) — `pecunia mcp` serves all nine modules over MCP/stdio; `logs.Actor` stamps agent writes as `ai`
- [0018-mcp-install-writes-agent-config-files](decisions/0018-mcp-install-writes-agent-config-files.md) — `pecunia mcp install <agent>`, offered at the end of `pecunia setup` — shells out to an agent's own CLI, or merges JSON directly for opencode
...
- [0026-backup-archives-to-a-directory-or-s3](decisions/0026-backup-archives-to-a-directory-or-s3.md) — `pecunia backup`: VACUUM INTO snapshot + notes in a tar.gz, optional age encryption, local, hand-rolled S3, Dropbox (PKCE OAuth) or Google Drive (PKCE + loopback listener, drive.file scope) provider, systemd user timer from `N/day`/`N/week`, restore moves live files aside as .bak — merged as PR #12; Dropbox added as a second provider in a follow-up PR
...
- [b4318b40](sessions/b4318b40-e452-4a98-bf96-2a937fcccfdc.md) — summary command (`pecunia s`), rendering fixes
...
- [c1d09b19](sessions/c1d09b19-d5ad-4ed0-b929-4d689f8bd290.md) — MCP server exposing all nine modules to an AI agent, plus an agent-config installer wired into `pecunia setup`; both committed together
...
- [ef2f43c3](sessions/ef2f43c3-8c61-4064-b047-63b0197a9abc.md) — rescued a `git pull` stuck mid-rebase (conflict in `wiki/index.md`, resolved by keeping both session lines and `git rebase --skip`), built `pecunia backup` end to end (directory/S3 archives, optional age encryption, systemd-timer scheduling), opened as PR #12 with CI confirmed green, then — after the user approved and merged PR #12 — added Dropbox as a second backup provider (PKCE OAuth consent flow, no SDK) on `feat/backup-dropbox`, opened as its own follow-up PR — see [[decisions/0026-backup-archives-to-a-directory-or-s3]]
