...
1. GET `api.github.com/repos/johnvilela/pecunia/releases` (public repo, unauthenticated; 5-minute-timeout client covers the download too).
...
Live-tested the bootstrap edge: a fake 0.0.1 build upgraded against the real v0.1.0 release — swap fine, but v0.1.0 predates `migrate`, exit 2. Since `db.Open()` migrates on the next run anyway, a failed exec prints "they will apply on the next pecunia run" and the upgrade still exits 0. Can't happen in the wild (any upgrade-capable binary only moves to migrate-capable ones), but the graceful path is tested (`TestUpgrade/a_failed_migrate_exec_defers...`).
...
TDD per [[rules/tdd]]. `cmd/upgrade_test.go`: tables for `versionLess`/`releasesSince`, `untarBinary` round-trip (mode 0755 asserted, missing-entry error), and `httptest` end-to-end — the served tarball's `pecunia` entry is a shell script that writes a marker when exec'd with `migrate`, proving the whole chain: changelog printed for all pending versions, binary replaced, migrations exec'd, no temp litter. Seams: package vars `releasesURL` and `selfExe`.
