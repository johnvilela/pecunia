...
Follow-up to [[decisions/0017-mcp-server-exposes-every-module-as-a-tool]], same session. User: "on the 'kakei mcp' does it install the MCP? If it doesnt: we may add a flag on the 'kakei setup' that install the MCP on the selected AI agent (codex, claude-code, opencode...)." A plan was written before any code (`~/.claude/plans/on-the-kakei-mcp-iridescent-pine.md`, outside the repo).

The installer landed as `kakei mcp install <agent>`, plus a step at the end of `kakei setup` that offers to run it — not a `setup` flag as first floated. `cmd/mcp_install.go` plus edits to `cmd/mcp.go`, `cmd/main.go` and `cmd/setup.go`.
...
- **opencode**, which has no add CLI — `kakei mcp install opencode` merges an `mcp` entry directly into `$XDG_CONFIG_HOME/opencode/opencode.json` (`TestOpencodeMerge`). Live-tested against a config that already had unrelated keys (`theme`, an existing `mcp.other` remote entry) — both preserved, kakei's own entry added alongside. Live-tested a second time against a config containing a `//`-style JSONC comment: the merge **refuses to touch it** and prints a paste-ready snippet instead, rather than risk corrupting a file it can't safely re-serialize. (This supersedes an earlier read of the same test as "the merge survives non-strict JSON" — the actual behavior is refuse-and-suggest, not silently write through the comment.)

## Other details

- No agent named on the command → an interactive picker over the supported agents (`core.Pick`).
- What gets registered is the **absolute path of the running binary** — a dev build's install therefore points an agent at the `dev` binary and its isolated `kakei.dev.db` ([[decisions/0004-dev-build-isolated-by-ldflags]]), not at the real `kakei`.
- `kakei setup` now ends with "Hook kakei up to an AI agent?" — declines silently with no TTY (verified with `echo | kakei setup`, exits clean), so scripted/piped setup runs are unaffected.
- Live-verified: `kakei mcp install claude-code` produced a `✔ Connected` entry in `claude mcp list` — Claude Code actually spawned the server and completed the MCP handshake. `codex` also installed and connected live. Both test entries were removed afterward since they pointed at a scratch build, not the real binary.
