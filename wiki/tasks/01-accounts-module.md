...
list: "pecunia accounts" or "pecunia ac"
create: "pecunia accounts new" or "pecunia ac n"
edit: "pecunia accounts edit {CODE|ID?}" or "pecunia ac e {CODE|ID?}"
delete: "pecunia accounts delete {CODE|ID?}" or "pecunia ac d {CODE|ID?}"
freeze: "pecunia accounts freeze {CODE|ID?}" or "pecunia ac f {CODE|ID?}"
details: "pecunia accounts {CODE|ID?}" or "pecunia ac {CODE|ID?}"
...
## Update (session pecunia-19): test suite, bugfixes, UI redesign, dev tooling, commit
...
- **Real bug caught by the tests**: `Store.Update` on a missing id silently returned `nil` instead of an error — `pecunia ac e` on a deleted account printed "updated". Fixed to return `ErrNotFound`, same as `Delete`.
