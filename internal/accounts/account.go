...
// Currency describes how an account's balance is stored and shown. Exp is the
// number of decimal places, which is also the power of ten between the stored
// integer (minor units) and the displayed amount: cents for fiat, satoshis for
// Bitcoin.
...
// Color is one of the twelve preset account colors. Rows store the name, so
// changing a hex value re-themes every account instead of breaking them.
type Color struct {
	Name string
	Hex  string
}
...
type Account struct {
	...
	Description string
	Color       string
	Balance     int64 // minor units, scaled by Currency().Exp
	Currency    string
	IsFrozen    bool
	CreatedAt   string
	UpdatedAt   string
...
func (a Account) Cur() Currency  { return CurrencyByCode(a.Currency) }
func (a Account) Col() Color     { return ColorByName(a.Color) }
func (a Account) Amount() string { return FormatAmount(a.Balance, a.Cur()) }
...
// digits as text — no float ever touches a balance — so BTC keeps all eight
